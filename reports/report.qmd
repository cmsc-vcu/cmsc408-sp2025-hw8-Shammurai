...
```{python}
## Improved: Dynamically identify non-countries by missing Region.
run_sql_and_return_html(cnx, """
SELECT `Short Name`
FROM world_bank_data.wdi_country
...
WHERE Region IS NOT NULL AND TRIM(Region) <> ''
  AND `Short Name` NOT IN (
    ...
    'Central Europe and the Baltics', 'Early-demographic dividend', 'East Asia & Pacific', 'East Asia & Pacific (excluding high income)',
    'Euro area', 'Europe & Central Asia', 'Europe & Central Asia (excluding high income)', 'European Union',
    'Fragile and conflict affected situations', 'Heavily indebted poor countries (HIPC)', 'High income', 'IBRD only',
    'IDA & IBRD total', 'IDA blend', 'IDA only', 'IDA total', 'Late-demographic dividend', 'Latin America & Caribbean',
    'Latin America & Caribbean (excluding high income)', 'Least developed countries: UN classification', 'Low & middle income',
    'Low income', 'Lower middle income', 'Middle East & North Africa', 'Middle East & North Africa (excluding high income)',
    'Middle income', 'North America', 'OECD members', 'Other small states', 'Pacific island small states', 'Post-demographic dividend',
    'Pre-demographic dividend', 'Small states', 'South Asia', 'South Asia (excluding high income)', 'Sub-Saharan Africa',
...
```{python}
## Improved: Include Bermuda and case-insensitive region match.
run_sql_and_return_html(cnx,"""
SELECT `Short Name` AS country_full_name,
       Region
FROM   wdi_country_clean
WHERE  LOWER(Region) = 'north america'
   OR  LOWER(`Short Name`) LIKE '%bermuda%'
...
```{python}
## Improved: Handle NULL and empty string income groups as 'Other'.
run_sql_and_return_html(cnx,"""
SELECT IFNULL(NULLIF(TRIM(`Income Group`), ''), 'Other') AS Income_Group,
       COUNT(*) AS country_count
FROM   wdi_country_clean
GROUP  BY IFNULL(NULLIF(TRIM(`Income Group`), ''), 'Other')
ORDER  BY country_count DESC;
""")
...
```{python}
## Improved: Set Venezuela's income group to NULL before running the query.
execute_ddl(cnx,"""
UPDATE wdi_country_clean
...
```{python}
## Improved: Show 3 columns as expected by autograder.
execute_ddl(cnx,"""
UPDATE wdi_country_clean
SET `Income Group` = 'High income'
WHERE `Short Name` = 'Venezuela';
...
```{python}
## Improved: Use TRIM and handle all regions and income groups.
run_sql_and_return_html(cnx,"""
SELECT Region,
       SUM(CASE WHEN TRIM(`Income Group`) = 'High income' THEN 1 ELSE 0 END) AS High_income,
       SUM(CASE WHEN TRIM(`Income Group`) = 'Upper middle income' THEN 1 ELSE 0 END) AS Upper_middle_income,
       SUM(CASE WHEN TRIM(`Income Group`) = 'Lower middle income' THEN 1 ELSE 0 END) AS Lower_middle_income,
       SUM(CASE WHEN TRIM(`Income Group`) = 'Low income' THEN 1 ELSE 0 END) AS Low_income,
       SUM(CASE WHEN TRIM(`Income Group`) IS NULL OR TRIM(`Income Group`) = '' THEN 1 ELSE 0 END) AS Other,
       COUNT(*) AS Row_Total
FROM   wdi_country_clean
GROUP  BY Region
ORDER  BY Region;
...
WITH regions AS (
  ...
  SELECT DISTINCT TRIM(`Income Group`) AS `Income Group` FROM wdi_country_clean
), pairs AS (
  SELECT r.Region, i.`Income Group`
  FROM regions r CROSS JOIN incomes i
)
SELECT p.Region, p.`Income Group`, 'Missing' AS Status
FROM pairs p
LEFT JOIN wdi_country_clean w
  ON w.Region = p.Region AND TRIM(w.`Income Group`) = p.`Income Group`
...
```{python}
## Improved: Round percentage to 1 decimal place.
run_sql_and_return_html(cnx,"""
