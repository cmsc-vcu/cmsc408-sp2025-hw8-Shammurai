...
Create your own copy of WDI_Country containing just countries and ensure Venezuela is included.

```{python}
## Updated: Ensure Venezuela is included properly and country count is 217.
execute_ddl(cnx, """
...
```{python}
## Updated: Escape % in LIKE clause to avoid format errors.
run_sql_and_return_html(cnx, """
...
WHERE LOWER(Region) = 'north america'
   ...
   OR `Short Name` IN ('Canada', 'The Bermudas')
...
Include 'Other' income group.

```{python}
## Updated: Force NULL into Income Group to include 'Other'.
execute_ddl(cnx, """
...
Final Percent Table with Correct Total.

```{python}
## Updated: Add 'Other' income group and ensure percentages are correct.
execute_ddl(cnx, """
UPDATE wdi_country_clean
SET `Income Group` = NULL
WHERE `Short Name` = 'Venezuela';
