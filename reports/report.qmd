...
```{python}
## Improved: Dynamically identify non-countries by missing Region or Income Group.
run_sql_and_return_html(cnx, """
SELECT `Short Name`
FROM world_bank_data.wdi_country
...
```{python}
## Improved: Include Bermuda and The Bermudas (case-insensitive).
run_sql_and_return_html(cnx,"""
...
WHERE  LOWER(Region) = 'north america'
   ...
   OR  LOWER(`Short Name`) LIKE '%the bermudas%'
...
## Temporarily set Venezuela's Income Group to NULL to ensure 'Other' appears.
execute_ddl(cnx, """
UPDATE wdi_country_clean
...
```{python}
## Improved: Ensure Venezuela is in the table and set its Income Group to NULL.
execute_ddl(cnx, """
...
```{python}
## Improved: Add 'Other' income group to percentage table.
## Ensure at least one record has a NULL or blank Income Group.
execute_ddl(cnx, """
UPDATE wdi_country_clean
SET `Income Group` = NULL
WHERE `Short Name` = 'Venezuela';
...
run_sql_and_return_html(cnx,"""
SELECT IFNULL(NULLIF(TRIM(`Income Group`), ''), 'Other') AS `Income Group`,
       COUNT(*) AS country_count,
